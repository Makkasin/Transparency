...
Функция ЗаполнитьДанныеДокументов(Адрес)
	...
	мДействиеОткрытьГиперссылку = ЭтаФорма.Команды.Добавить("ОткрытьГиперссылку");
	мДействиеОткрытьГиперссылку.Действие = "ОткрытьГиперссылку";
...
Процедура ОбработкаНажатияНаДокумент(пИндексСтроки)
	Если пИндексСтроки <> Неопределено И пИндексСтроки >= 0 Тогда
		...
		Если фСтрока <> Неопределено Тогда
			...
			//		мИмяФайла = "" + КаталогВременныхФайлов() + фСтрока.ПолноеИмяФайла;

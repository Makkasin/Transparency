...
Процедура ОткрытьЖЦ(Команда)
	...
	Если СокрЛП(тблБаз[0].ИНН) <> "" Тогда
		мПараметры = Новый Структура("РежимДляЧтения, ГаражныйНомер, ИНН", Истина, фГарНомер, тблБаз[0].ИНН);
		ОткрытьФорму("Справочник.уатТС.Форма.ФормаЭлементаУпр", мПараметры);
	Иначе
		ПоказатьПредупреждение(, "Не указан ИНН организации, которой принадлежит ТС. Обратитесь в тех.поддержку!");
	КонецЕсли;
КонецПроцедуры
...
Функция ЗаполнитьДанныеДокументов(Адрес)
	...
	Если мДанные <> Неопределено Тогда
		...
		Для Каждого мДокумент Из мМассивДокументов Цикл
			...
			мГиперссылка.ЦветТекста = ЦветаСтиля.ЦветТекстаФормы;

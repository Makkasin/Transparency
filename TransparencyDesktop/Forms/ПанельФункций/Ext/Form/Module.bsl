...
Функция ПолучитьТаблицуОрганизаций()
	мВременныйАдрес = ПоместитьВоВременноеХранилище(Неопределено);
	глСервер.ВыполнитьAPI("10.8.85.12", 80, "/chat_pl/hs/flowAPI/GETTRANSPDATATABLE", мВременныйАдрес, Неопределено);
...
Процедура ДобавитьКнопку(пИмя, пЗаголовок, пИмяГруппы, пИмяКоманды, пТекстПодсказки = "", пВысота, пЦвет = Неопределено)
	...
	НовыйЭлемент.ЦветРамки = WebЦвета.БледноЗолотистый;
	НовыйЭлемент.Шрифт = Новый Шрифт(НовыйЭлемент.Шрифт, , 10, Истина);
...
Процедура ОбновитьКнопкиИнтерфейса()
	...
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.Легковые.Наименование, WebЦвета.ЦветМорскойВолны);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.Автобусы.Наименование, WebЦвета.Коралловый);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.Грузовые.Наименование, WebЦвета.ВесеннеЗеленый);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.Спецтехника.Наименование, WebЦвета.ЗеленоЖелтый);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.Экскаваторы.Наименование, WebЦвета.Сливовый);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.Трактора.Наименование, WebЦвета.Оранжевый);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.ПрицепыИПолуприцепы.Наименование, WebЦвета.НасыщенноНебесноГолубой);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.Прочие.Наименование, WebЦвета.НейтральноПурпурный);
	мСоответствиеЦветовГрупп.Вставить(Справочники.уатТипыТС.ОбородованиеСобственныеНужды.Наименование, WebЦвета.БледноМиндальный);
	...
	Для Итр = 1 По 2 Цикл
		...
		Для Джитр = 1 По мЧислоДвойныхГрупп Цикл
			...
			Если мРодительВерхнегоУровня = "Прочие" Тогда
				ДобавитьКнопку("" + мРодительВерхнегоУровня + "ПП", "Прочие", "ГруппаГГ" + мИндекс, "ОткрытьОтчет", "Все ТС без группы", 2, мСоответствиеЦветовГрупп.Получить(мРодительВерхнегоУровня));
			КонецЕсли;
		КонецЦикла;

...
Функция ДобавитьГруппу(пИмя, пРодитель, пГоризонтальная = Ложь, пВыделить = Ложь, пЗаголовок = "")
	...
	мНоваяГруппа.Группировка = ?(пГоризонтальная, ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда, ГруппировкаПодчиненныхЭлементовФормы.Вертикальная);
...
Процедура ОбновитьКнопкиИнтерфейса()
	...
	Для Итр = 1 По 2 Цикл
		...
		Для Джитр = 1 По мЧислоДвойныхГрупп Цикл
			...
			мИндекс = ((Итр - 1) * 4) + Джитр;
			Если мИндекс > мМассивРодителейВНаличии.Количество() Тогда
				Прервать;
			КонецЕсли;
			мРодительВерхнегоУровня = мМассивРодителейВНаличии.Получить(мИндекс - 1);
			мПодГруппа = ДобавитьГруппу("ГруппаВ" + мИндекс, мНоваяГруппа, Ложь, Истина, "" + Справочники.уатТипыТС[мРодительВерхнегоУровня].Наименование);
			...
			//Если Джитр <> мЧислоДвойныхГрупп Тогда
			...
			//внутри горизонтальной - 4 вертикальных
...
Процедура ПоказатьОтчетНаКлиенте()
	...
	Иначе
		мМассивТиповРодителя = фСоответствиеТиповРодителямВерхнегоУровня.Получить(фРодитель);
		мРодитель = мМассивТиповРодителя[фИндексКнопки - 1];
		ПоказатьОтчет(Неопределено, мРодитель, фФильтрНаКонсервации);
	КонецЕсли;
КонецПроцедуры
